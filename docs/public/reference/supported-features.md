...
| Gear Standard Library | Supported / bounded by family | Supported external/internal standard involute spur gears; bounded straight bevel/miter and ratchet/pawl; typed `Interface<Gear>` compatibility and ideal ratios; deterministic manifold AP242. No contact, dynamics, ratings, helical/worm, spiral-bevel, or qualified hub/keyway geometry. See the [gear guide](../firmament/gears.md). |
| Assembly Interface bundles and Subassemblies | Supported / bounded | Compiler-owned `Fixed`, `Axial`, and `Revolute` expansion; explicit `Custom` atomic bundles; compile-time relative `Include`; reusable nested definitions; private-by-default internals; exposed semantic ports that preserve type, including Gear; stable hierarchical paths and dependency hashes. Include aliases are deferred. See [assemblies](../firmament/assemblies.md). |
...
| Revolve | Bounded (X1) | One bounded outer line/arc Profile, explicit coplanar Axis, `0 < \|Angle\| <= full`, signed direction, safe Base construction; no multi-turn, axis-crossing, general axis-touching, inner loops, Add/Remove, or skew projection |
| Profile/Path `|>` composition | Supported (LANG-PIPELINE-X1) | finite qualified span chains, automatic continuity orientation, `Reverse`, `As`, validating `Close`, real-loop-only `TraceLoop`, inherited identity/provenance; no general function pipe or control flow |
| Wire coils | Bounded (WIRE-X1) | Semantic `Helix` (`AxisCoil` compatibility spelling) plus Cylinder/Frustum/Sphere SurfaceCoil, explicit handedness/clearance, constant circular wire, evaluable winding authority, deterministic cubic non-rational STEP tube; no variable pitch/section, arbitrary support, intentional contact, or dynamics |
| Formed Wire | Bounded (WIRE-X0) | One open, continuous, unbranched constant circular wire; explicit 3D StartFrame; ordered Straight and signed planar Bend operations through ±180° in current local Up/Right planes; exact line/arc centerline, ideal cut length/volume/mass, analytic cylinders/tori/caps, terminals, and fail-closed no-contact validation. No coils, knots, branches, variable section, springback, or intentional contact. See [guide](../firmament/wire-form.md). |
| SectionChain loft substrate | Bounded (SURF-X4 subset) | Firmament authoring lowers into ordered framed, same-topology semantic SectionChain IR; explicit G0 ruled or default G1 local polynomial transitions; shared edges and tangent fields; Cap/Open; strict pcurves; deterministic intersection checks; persistent typed BodyState Add/Remove through documented planar housing lanes; no generic CSG authoring, rational product surface, or faceted fallback. See [Section chains](../firmament/section-chains.md). |
| `Loft` and `Loft<Hollow>` | Bounded two-section circle/ellipse profiles | Two framed outlines, straight ruled generators, `CommonRay` correspondence, and optional `Twist` (-180° to 180°). `Loft` makes a capped solid; `Loft<Hollow>` adds nominal wall thickness and a narrow explicit seam. Both lower through SectionChain to closed AP242 solids. See [Loft](../firmament/loft.md). |
...
| Boss | Bounded | First-class connected finite profile addition; relative `On: Top` resolves the highest current semantic support under the footprint, named `<feature>.Top`/`Base.Top` is explicit, ambiguity fails typed; positive height; lowers through existing `Add`; no arbitrary solid union |
...
| `AddOffset<Prism>` / `RemoveOffset<Prism>` | Bounded (X1 Prism subset) | Explicit active Compose target, closed Profile tool, whole Top or bounded `Span<Plane>` support, `+Z` direction; Add uses positive Height, Remove uses positive Depth or ThroughAll; edge-crossing removals allowed on whole Top; Cylinder/Sphere deferred and rejected typed |
...
| Post-Preview-3 feature families | X0 exception | Circular Sweep, WIRE-X0 Formed Wire, and Standard Products Paperclip are bounded X0 additions; other new geometry/PMI/FEA/Forge families remain frozen |
| General smooth/rail/topology-changing loft and freeform surface features | Not in Preview 3 | The bounded two-section lofts, G1 SectionChains, and WireForm `Helix` do not qualify arbitrary smooth loft, guide rails, topology-changing correspondence, arbitrary dumb-solid editing, G2 continuity, or surface networks. |
