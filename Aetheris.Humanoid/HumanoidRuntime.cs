...
public static class HumanoidMorphing
{
    public static CanonicalHumanoid Apply(CanonicalHumanoid h,IReadOnlyDictionary<HumanoidMorphId,double> values)
    {
        ...
        var vertices=h.Surface.Vertices.Select(v=>v with{Position=Map(v.Position,v.Region)}).ToArray();var oldJoints=h.Skeleton.Joints;var globals=oldJoints.Select(j=>{var p=Map(new(j.GlobalBind.M41,j.GlobalBind.M42,j.GlobalBind.M43),RegionForJoint(j.Kind));if(!Matrix4x4.Decompose(j.GlobalBind,out _,out var orientation,out _))throw new InvalidDataException("Morphed joint frame is not rigid: "+j.Kind);var m=Matrix4x4.CreateFromQuaternion(Quaternion.Normalize(orientation));m.Translation=new((float)p.X,(float)p.Y,(float)p.Z);return m;}).ToArray();var joints=new HumanoidJoint[oldJoints.Count];for(var i=0;i<joints.Length;i++){var old=oldJoints[i];var global=globals[i];var local=old.ParentIndex is int pi?global*Inverse(globals[pi]):global;if(!Matrix4x4.Decompose(local,out var scale,out var rotation,out var translation)||Vector3.Distance(scale,Vector3.One)>1e-4f||!Matrix4x4.Invert(global,out var inv))throw new InvalidDataException("Morphed joint frame is not rigid: "+old.Kind);joints[i]=old with{LocalRest=new(new(translation.X,translation.Y,translation.Z),Quaternion.Normalize(rotation)),GlobalBind=global,InverseBind=inv};}
        var skeleton=h.Skeleton with{Joints=joints};var surface=h.Surface with{Vertices=vertices};var frame=values.TryGetValue(HumanoidMorphId.Height,out var requestedHeight)?h.Frame with{NormalizedHeightMm=requestedHeight}:h.Frame;var updated=h with{Frame=frame,Surface=surface,Skeleton=skeleton,ShapeRevision=h.ShapeRevision+1,PoseState=new(h.Skeleton.RestPoseId,[],HumanoidTransform.Identity)};return updated with{Attachments=updated.Attachments.Select(site=>site with{NeutralFrame=site.NeutralFrame with{Position=CanonicalAdultTemplate.Resolve(surface,site.PositionBinding)}}).ToArray()};
    }
    ...
    private static Matrix4x4 Inverse(Matrix4x4 matrix)=>Matrix4x4.Invert(matrix,out var inverse)?inverse:throw new InvalidDataException("Singular morphed parent frame.");
}
