...
public static class CliRunner
{
    ...
    private static int RunModuleShowcase(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        File.WriteAllText(Path.Combine(output,"validation-report.json"),JsonSerializer.Serialize(evidence,JsonOptions));File.WriteAllText(Path.Combine(output,"surfacing-gallery-inspection.json"),JsonSerializer.Serialize(galleryReports,JsonOptions));stdout.WriteLine(JsonSerializer.Serialize(new{output,saddleStep=saddlePath,pipeStep=pipePath,saddleHash=Hash(saddleStep.Value),pipeHash=Hash(pipeStep.Value),surfacingGallery=galleryReports.Count},JsonOptions));return 0;
    }
