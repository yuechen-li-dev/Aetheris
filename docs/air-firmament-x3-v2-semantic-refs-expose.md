...
`fixtures/FirmamentV2/SemanticRefs/valid/named-box-faces-v2.valid.firmfixture` is now parser-backed and expected to reach `feature-air` with four exposures. Focused invalid fixtures cover duplicate aliases, invalid axes, raw backend IDs, `=>` outside expose blocks, and unsupported `edge(...)` selectors.
