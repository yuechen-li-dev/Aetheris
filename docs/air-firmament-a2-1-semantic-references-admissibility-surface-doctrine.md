...
A2.1 adds metadata-only design fixtures under `fixtures/FirmamentV2/` for semantic references, feature-output aliases, forbidden backend IDs, degenerate dimensions, shell collapse, ruled/offset surface examples, and pattern-as-record examples. Until a V2 parser exists, valid future examples are classified as `not-implemented` with `firmament-v2-parser-not-ready`, and invalid examples are classified by metadata diagnostics such as `firmament-degenerate-dimension`, `firmament-shell-thickness-collapses-body`, and `firmament-raw-backend-id-reference-forbidden`. They must not be run through the V1 parser as random parse failures.
