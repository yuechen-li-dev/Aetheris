...
public sealed class CirRegionPlannerTests
{
    ...
    public void Planner_ClassifiesInsideRegion()
    {
        ...
        var context = BuildMixedContext(new FieldInterval(-2d, -0.1d), SdfRegionClassification.Inside, depth: 0, maxDepth: 4);
    ...
    {
        var planner = new CirRegionPlanner();
        var context = BuildMixedContext(new FieldInterval(0.1d, 2d), SdfRegionClassification.Outside, depth: 0, maxDepth: 4);
    ...
    {
        var planner = new CirRegionPlanner();
        var context = BuildMixedContext(new FieldInterval(-0.25d, 0.25d), SdfRegionClassification.Mixed, depth: 1, maxDepth: 4);
    ...
    {
        var planner = new CirRegionPlanner();
        var context = BuildMixedContext(new FieldInterval(-0.25d, 0.25d), SdfRegionClassification.Mixed, depth: 3, maxDepth: 3);
    ...
    {
        var planner = new CirRegionPlanner();
        var context = BuildMixedContext(new FieldInterval(-0.25d, 0.25d), SdfRegionClassification.Mixed, depth: 0, maxDepth: 4);
    ...
    public void Planner_UsesRealTapeInterval()
    {
        ...
        var region = new SdfBounds(new Point3D(1.8d, -0.3d, -1d), new Point3D(2.2d, 0.3d, 1d));
        ...
        Assert.Equal(CirRegionPlanAction.Subdivide, plan.Action);
        Assert.Equal(SdfRegionClassification.Mixed, plan.Classification);
        Assert.True(plan.Interval.MinValue <= 0d && plan.Interval.MaxValue >= 0d);
    }

    private static CirRegionPlanContext BuildMixedContext(FieldInterval interval, SdfRegionClassification classification, int depth, int maxDepth)
    {
        var options = new CirRegionPlannerOptions(MaxDepth: maxDepth, DirectSampleThreshold: 32, MinimumRegionExtent: 0.1d);
        var region = new SdfBounds(new Point3D(-1d, -1d, -1d), new Point3D(1d, 1d, 1d));
