...
function anchor(entity: CadmataEntity): [number, number, number] | null {
	...
	if (geometry?.type === "polyline" && geometry.points.length) {
		const p = geometry.points[geometry.points.length - 1];
		return [p.x, p.y, p.z];
	}
	return null;
}
...
export function PmiAnnotationLayer({
	...
	return (
		<group name="semantic-pmi-annotations">
			{artifact.entities
				.filter((entity) => entity.kind === "HoleDiameter" || entity.kind === "Datum")
				.map((entity) => {
					const target = anchor(entity);
					if (!target) return null;
					const isDatum = entity.kind === "Datum";
					const selected = selectedIds.has(entity.stableId);
					const offset: [number, number, number] = [
						target[0] + (isDatum ? -12 : 12),
						target[1] + 2.4,
						target[2] + 0.2,
					];
					...
					return (
						<group key={entity.stableId}>
							<Line
								points={[target, offset]}
								color={theme.annotation.leader}
								lineWidth={selected ? 2.5 : 1.3}
							/>
							<Html position={offset} center style={{ pointerEvents: "auto" }}>
								<button
									className="pmi-callout"
									type="button"
									onClick={(event) => {
								...
								>
									<span>{isDatum ? `DATUM ${entity.label}` : formatPmiLabel(entity)}</span>
									{!isDatum && entity.metadata?.datumRefs ? (
										<small>{String(entity.metadata.datumRefs)}</small>
									) : null}
								</button>
							</Html>
						</group>
