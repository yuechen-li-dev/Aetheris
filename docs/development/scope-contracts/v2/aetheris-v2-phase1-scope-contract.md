...
V2-PHASE1-P2 is implemented for robust AP242 export of record-shaped `pmi` `datum` and toleranced `diameter` records over existing canonical InlineStep models. The proof fixture is `fixtures/InlineStep/valid/inline-step-v2-record-pmi-datum-diameter-step-verified.valid.firmfixture`; it imports the small canonical through-hole STEP model, recognizes a datum plane and hole region, binds a toleranced record `let`, exports semantic AP242 datum/diameter/tolerance evidence through `Step242Exporter`, and reimports the result in tests.
