...
Canonical policy: topology edge start/end and `ORIENTED_EDGE.orientation` determine coedge traversal; `EDGE_CURVE.same_sense` only maps curve parameter direction to topology endpoints; `FACE_OUTER_BOUND`/`FACE_BOUND` carry explicit loop role; `FACE_BOUND.orientation` is retained as STEP source evidence. Historical note: the former statement that `ADVANCED_FACE.same_sense` applies once is superseded by STEP-ORIENTATION-X1; it is now cross-check evidence only, and the importer derives canonical face orientation from topology and geometry. Import must not mutate `EDGE_LOOP` order to satisfy a local winding preference.
