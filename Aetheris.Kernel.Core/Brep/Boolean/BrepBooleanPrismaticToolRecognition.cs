...
internal static class BrepBooleanPrismaticToolRecognition
{
    public static bool TryRecognize(BrepBody body, ToleranceContext tolerance, out SupportedPrismaticSubtractTool tool, out string reason)
    {
        ...
        if (points.Count == 0)
        {
            reason = "prismatic tool recognition requires vertex points or line-edge bindings.";
            return false;
        }

        if (points.Count < 6)
        {
            reason = "prismatic tool recognition requires at least six vertices.";
            return false;
        }
        ...
        {
            reason = "prismatic tool recognition requires positive Z span.";
            return false;
        }
        ...
        {
            reason = "prismatic tool recognition requires planar top and bottom loops with at least three vertices.";
            return false;
        }
        ...
        {
            reason = "prismatic tool recognition requires matching top/bottom XY loop cardinality.";
            return false;
        }
        ...
        foreach (var xy in bottomUnique)
        {
            ...
            {
                reason = "prismatic tool recognition requires matching top/bottom XY vertices.";
                return false;
            }
        }

        var footprint = OrderLoop(bottomUnique);
        tool = new SupportedPrismaticSubtractTool(
