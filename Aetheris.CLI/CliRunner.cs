...
public static class CliRunner
{
    ...
    private static int RunVerify(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        var evidenceDir = Path.GetFullPath(evidenceRoot ?? Path.Combine("artifacts", "local", "verification", fixtureName, hash));
    ...
    private static int RunBuild(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        if (json)
        {
            stdout.WriteLine(JsonSerializer.Serialize(new
            {
                ...
                perforation = build.Value.Export.Perforation,
    ...
    private static int RunSculptingBuild(string sourcePath, string? outPath, bool json, TextWriter stdout, TextWriter stderr)
    {
        ...
        if (json) stdout.WriteLine(JsonSerializer.Serialize(new
        {
            ...
            validationEvidence = compile.OutputState.ValidationEvidence, surfacePatches = compile.OutputState.SurfacePatches, blendJudgment = compile.OutputState.BlendJudgment, surfaceInventory = export.Inventory, rationalNurbs = export.Inventory.RationalNurbs,
        ...
        {
            stdout.WriteLine($"Built {Path.GetFileName(sourcePath)}"); stdout.WriteLine($"STEP: {output}"); stdout.WriteLine($"State: {compile.OutputState.AuthoredName} ({compile.OutputState.StateId}) <- {compile.OutputState.PredecessorStateId}"); stdout.WriteLine($"Geometric delta: {deltaPath}"); stdout.WriteLine($"STEP surfaces: Plane={export.Inventory.Plane}, Cylinder={export.Inventory.Cylinder}, NonRationalBSpline={export.Inventory.NonRationalBSpline}, RationalNURBS={export.Inventory.RationalNurbs}");
            if (compile.OutputState.BlendJudgment is { } judgment)
            {
                stdout.WriteLine($"Blend judgment: {judgment.Request}");
                foreach (var candidate in judgment.Candidates)
                    stdout.WriteLine($"  {candidate.CandidateId}: {candidate.Disposition}" + (candidate.Metrics is null ? $" — {candidate.RejectionReason}" : $"; utility={candidate.Metrics.Utility:F6}, fairness={candidate.Metrics.Fairness:F6}, curvature={candidate.Metrics.CurvatureSmoothness:F6}, compactness={candidate.Metrics.Compactness:F6}, complexity={candidate.Metrics.Complexity:F6}"));
    ...
    private static int RunSheetMetalAuthoredBuild(string sourcePath, string? outPath, bool json, TextWriter stdout, TextWriter stderr)
    {
        ...
        var semanticPmi=authored.Spec?.Manufacturing.ToStep242SemanticPmi(authored.Part)??[];
        var export=Step242Exporter.ExportBody(authored.Part.FormedBody,semanticPmi,new Step242ExportOptions{BrepExportPreflightMode=BrepExportPreflightMode.Enforce,ProductName=authored.Spec?.Name??"SheetMetalPart"});
        ...
        if(json)stdout.WriteLine(JsonSerializer.Serialize(new{command="build",success=true,input=Path.GetFullPath(sourcePath),output,domain="SheetMetal",part=new{authored.Part.StableId,authored.Part.Thickness,authored.Part.Material,regions=authored.Part.Regions.Count,bends=authored.Part.Bends.Count,features=authored.Part.Features.Count},manufacturing=new{release=authored.Spec?.Manufacturing.ReleaseName,materialSpecification=authored.Spec?.Manufacturing.MaterialSpecification,semanticPmi=semanticPmi.Count},flatPattern=new{authored.FlatPattern!.Status,authored.FlatPattern.Bounds,authored.FlatPattern.DeterministicHash}},JsonOptions));
        else{stdout.WriteLine($"Built {Path.GetFileName(sourcePath)}");stdout.WriteLine($"STEP: {output}");stdout.WriteLine($"Model: {authored.Part.StableId} (SheetMetal, {authored.Part.Bends.Count} bends)");if(authored.Part.Material is not null)stdout.WriteLine($"Material: {authored.Part.Material}");}
        return 0;
    }
    ...
    private static int RunSheetMetal(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        SheetMetalPartIr? part;SheetThicknessRecognition? thickness=null;TimeSpan importTime=TimeSpan.Zero,recognitionTime=TimeSpan.Zero;SheetMetalAuthoringTimings? authoredTimings=null;SheetMetalConstructionSpec? authoredSpec=null;IReadOnlyList<Aetheris.Kernel.Firmament.FirmamentV2.ConceptIrTemplateInstantiation> templateInstantiations=[];IReadOnlyList<SheetMetalDiagnostic> recognitionDiagnostics;
        var source=File.ReadAllText(input);if(SheetMetalFirmament.LooksLikeSheetMetal(source))
        {
            var authored=SheetMetalFirmament.Compile(source,input);part=authored.Part;authoredSpec=authored.Spec;authoredTimings=authored.Timings;templateInstantiations=authored.TemplateInstantiations??[];recognitionDiagnostics=authored.Diagnostics;if(!authored.IsSuccess||part is null){WriteFailure(authored.Diagnostics);return 1;}
        ...
        var allDiagnostics=recognitionDiagnostics.Concat(flat.Diagnostics).GroupBy(x=>(x.Code,x.Severity,x.Message)).Select(x=>x.First()).ToArray();
        var conceptPaths=authoredSpec is null?Array.Empty<SheetMetalConceptPath>():SheetMetalConceptPaths.Inspect(authoredSpec,part,flat);
        if(operation=="paths")
        {
            if(authoredSpec is null){stderr.WriteLine("sheetmetal paths requires authored or reconstructed Firmament, not recovered STEP topology.");return 1;}
            if(json)stdout.WriteLine(JsonSerializer.Serialize(new{command="sheetmetal paths",success=true,input,paths=conceptPaths},JsonOptions));
            else foreach(var path in conceptPaths)stdout.WriteLine($"{path.Path}  {path.Kind}  [{string.Join(", ",path.Capabilities)}]");
            return 0;
        }
        ...
        var pmiIndicators=authoredSpec is null
            ? Regex.Matches(source,"(DRAUGHTING_MODEL|GEOMETRIC_TOLERANCE|DATUM_FEATURE|ANNOTATION_OCCURRENCE|SHAPE_ASPECT)",RegexOptions.IgnoreCase).Count
            : authoredSpec.Manufacturing.Datums.Count+authoredSpec.Manufacturing.Dimensions.Count+authoredSpec.Manufacturing.GeometricTolerances.Count+authoredSpec.Manufacturing.Annotations.Count;
        var exactValidation=flat.ExactBlankContour is null?null:PlanarContourKernel.Validate(flat.ExactBlankContour);
        var report=new{command=$"sheetmetal {operation}",success=true,input,sourceAuthority=SheetMetalFirmament.LooksLikeSheetMetal(source)?(source.Contains("Intent: Reconstructed",StringComparison.OrdinalIgnoreCase)?"Firmament reconstructed engineering authority":source.Contains("RecoveredRegion",StringComparison.Ordinal)?"Firmament recovered sheet-metal semantics":"Firmament authored semantics"):"STEP formed BRep (not mutated)",recognitionStatus=part.RecognitionStatus,material=new{authored=part.Material,manufacturingSpecification=authoredSpec?.Manufacturing.MaterialSpecification},thickness=new{nominal=part.Thickness,tolerance=authoredSpec?.Manufacturing.ThicknessTolerancePlus??thickness?.Tolerance,evidencePairs=thickness?.SourcePairs.Count(p=>p.Admitted)??0,outlierFaces=thickness?.OutlierFaceIds.Count??0},geometry=new{bodies=body?.Topology.Bodies.Count()??0,shells=body?.Topology.Shells.Count()??0,faces=body?.Topology.Faces.Count()??0,edges=body?.Topology.Edges.Count()??0,vertices=body?.Topology.Vertices.Count()??0,supportFamilies,pmiIndicators},manufacturing=authoredSpec?.Manufacturing,sheetMetal=new{baseRegion=part.BaseRegionId,regions=part.Regions.Select(r=>new{r.StableId,r.Kind,r.ApproximateArea,sourceFaces=r.Source.FaceIds}),attachmentPaths=(part.AttachmentPaths??[]).Select(p=>new{p.StableId,p.OwningRegionId,p.CarrierPath,p.Inset,length=(p.End-p.Start).Length,p.SpanOffset,p.Start,p.End,p.Tangent,p.InPlaneNormal,p.RegionNormal,p.Capabilities}),bends=part.Bends.Select(b=>new{b.StableId,b.AxisOrigin,b.AxisDirection,angleDegrees=b.BendAngleRadians*180/Math.PI,b.InsideRadius,b.Direction,b.AdjacentRegionA,b.AdjacentRegionB,sourceFaces=b.Source.FaceIds}),cuts=part.Features.Select(f=>new{f.StableId,f.Kind,f.OwningRegionId,f.Diameter,sourceFaces=f.Source.FaceIds}),reliefs=part.Reliefs?.Count??0,semanticLayout=authoredSpec?.SemanticLayout,conceptPaths,templateInstantiations},flatPattern=new{flat.Status,MaterialArea=flat.MaterialArea,BoundingArea=flat.BoundingArea,flat.Bounds,bendLines=flat.BendLines.Count,cuts=flat.CutLoops.Count,reliefs=flat.ReliefLoops?.Count??0,exactBlank=flat.ExactBlankContour is not null,outerLoops=flat.ExactBlankContour is null?0:1,innerLoops=flat.ExactBlankContour?.InnerLoops.Count??0,connectedMaterialRegions=flat.ExactBlankContour is null?0:1,selfIntersects=exactValidation?.Diagnostics.Any(x=>x.Code=="planar-contour-self-intersection")??false,exactContoursValid=exactValidation?.IsValid??false,fallbackUsed=authoredSpec is not null&&flat.ExactBlankContour is null,kFactor=flat.Policy.KFactor,flat.DeterministicHash,svg=output,step=flatStepOutput,firmament=firmamentOutput},dfm=new{dfm.Overall,findings=dfm.Findings},diagnostics=allDiagnostics,timingsMilliseconds=new{parse=authoredTimings?.ParseMilliseconds,semanticResolve=authoredTimings?.SemanticResolveMilliseconds,formedLower=authoredTimings?.FormedLowerMilliseconds,authoredFlatLower=authoredTimings?.FlatLowerMilliseconds,import=importTime.TotalMilliseconds,recognition=recognitionTime.TotalMilliseconds,flatten=flattenClock.Elapsed.TotalMilliseconds}};
        if(json)stdout.WriteLine(JsonSerializer.Serialize(report,JsonOptions));else
        {
            stdout.WriteLine($"Sheet Metal: {part.RecognitionStatus} ({report.sourceAuthority})");if(part.Material is not null)stdout.WriteLine($"Material: {part.Material}");stdout.WriteLine($"Thickness: {part.Thickness:G8} mm{(thickness is null?" authored/recovered":$" ± {thickness.Tolerance:G4} mm, {thickness.SourcePairs.Count(p=>p.Admitted)} paired supports")}");stdout.WriteLine($"Regions: {part.Regions.Count} ({part.Regions.Count(r=>r.Kind==SheetRegionKind.Planar)} planar, {part.Regions.Count(r=>r.Kind==SheetRegionKind.CylindricalBend)} cylindrical)");stdout.WriteLine($"Bends: {part.Bends.Count}; cuts: {part.Features.Count}; base: {part.BaseRegionId}");foreach(var bend in part.Bends)stdout.WriteLine($"  {bend.StableId}: {bend.BendAngleRadians*180/Math.PI:G5} deg, R{bend.InsideRadius:G6}, {bend.Direction}, {bend.AdjacentRegionA} -> {bend.AdjacentRegionB}");stdout.WriteLine($"Flat pattern: {flat.Status}; MaterialArea={(flat.MaterialArea is { } materialArea?$"{materialArea:G10} mm^2":"unavailable")}; BoundingArea={(flat.BoundingArea is { } boundingArea?$"{boundingArea:G10} mm^2":"unavailable")}; {(flat.Bounds is null?"no bounds":$"{flat.Bounds.Width:G8} x {flat.Bounds.Height:G8} mm")}; K={flat.Policy.KFactor:G4}; hash={flat.DeterministicHash}");if(flatStepOutput is not null)stdout.WriteLine($"STEP: {flatStepOutput}");if(firmamentOutput is not null)stdout.WriteLine($"Firmament: {firmamentOutput}");if(output is not null)stdout.WriteLine($"SVG: {output}");foreach(var d in allDiagnostics)stdout.WriteLine($"  [{d.Severity}] {d.Code}: {d.Message}");
        }
    ...
    {
        if(args.Length<2||args.Skip(2).Any(x=>x is not ("--json" or "--semantic"))){stderr.WriteLine(SheetMetalUsage);return 1;}
        var referencePath=Path.GetFullPath(args[0]);var nativePath=Path.GetFullPath(args[1]);var json=args.Contains("--json",StringComparer.Ordinal);var semantic=args.Contains("--semantic",StringComparer.Ordinal);
        ...
        var comparison=RecoveredFlatComparer.Compare(source,native.FlatPattern);var semanticComparison=semantic?SemanticSheetMetalComparer.CompareFlat(source,native.Part!,native.FlatPattern,sourcePart:model.DetectedPart):null;var report=new{command="sheetmetal compare-flat",success=comparison.Status!=SheetMetalComparisonStatus.Fail,reference=referencePath,native=nativePath,comparison,semanticComparison};
        if(json)stdout.WriteLine(JsonSerializer.Serialize(report,JsonOptions));else{stdout.WriteLine($"Recovered source flat -> native flat: {comparison.Status}");stdout.WriteLine($"source->native RMS/p95/max: {comparison.SourceToNative.Rms:G6}/{comparison.SourceToNative.P95:G6}/{comparison.SourceToNative.Maximum:G6} mm");stdout.WriteLine($"native->source RMS/p95/max: {comparison.NativeToSource.Rms:G6}/{comparison.NativeToSource.P95:G6}/{comparison.NativeToSource.Maximum:G6} mm");foreach(var item in comparison.LocalizedDifferences)stdout.WriteLine($"  {item}");if(semanticComparison is not null){stdout.WriteLine($"Semantic targets: {semanticComparison.Status}; tolerance {semanticComparison.Tolerance:G6} mm");foreach(var target in semanticComparison.Targets.Where(x=>x.Status!=SemanticGeometryComparisonStatus.Pass).Take(20))stdout.WriteLine($"  {target.Status} {target.SemanticPath}: max {Math.Max(target.Metrics.SourceToNative.Maximum,target.Metrics.NativeToSource.Maximum):G6} mm; {target.Classification}");}}
    ...
    {
        if(args.Length<2||args.Skip(2).Any(a=>a is not ("--json" or "--semantic"))){stderr.WriteLine(SheetMetalUsage);return 1;}var sourcePath=Path.GetFullPath(args[0]);var intentPath=Path.GetFullPath(args[1]);var json=args.Contains("--json",StringComparer.Ordinal);var semantic=args.Contains("--semantic",StringComparer.Ordinal);
        ...
        var source=Load(sourcePath);var intent=Load(intentPath);if(source is null||intent is null){stderr.WriteLine("Sheet Metal compare could not compile/recover both inputs.");return 2;}var comparison=SheetMetalIntentComparer.Compare(source,intent);var semanticComparison=semantic?SemanticSheetMetalComparer.CompareFormed(source,intent):null;
        if(json)stdout.WriteLine(JsonSerializer.Serialize(new{command="sheetmetal compare",success=comparison.Status is SheetMetalComparisonStatus.Pass or SheetMetalComparisonStatus.PassWithKnownDifferences or SheetMetalComparisonStatus.NeedsReview,source=sourcePath,intent=intentPath,comparison,semanticComparison},JsonOptions));
        else{stdout.WriteLine($"Sheet Metal intent comparison: {comparison.Status}");stdout.WriteLine($"Thickness residual: {comparison.ThicknessResidual:G6} mm");stdout.WriteLine($"Formed source->intent RMS/p95/max: {comparison.SourceToIntent.Rms:G6}/{comparison.SourceToIntent.P95:G6}/{comparison.SourceToIntent.Maximum:G6} mm");stdout.WriteLine($"Formed intent->source RMS/p95/max: {comparison.IntentToSource.Rms:G6}/{comparison.IntentToSource.P95:G6}/{comparison.IntentToSource.Maximum:G6} mm");foreach(var b in comparison.Bends)stdout.WriteLine($"  {b.SourceBendId} -> {b.IntentBendId}: axis {b.AxisResidual:G6} mm, axis-angle {b.AxisAngleResidualDegrees:G6} deg, bend-angle {b.BendAngleResidualDegrees:G6} deg, radius {b.RadiusResidual:G6} mm, adjacency {b.AdjacencyMatches}, {b.Status}");if(semanticComparison is not null)foreach(var t in semanticComparison.Terminations)stdout.WriteLine($"  {t.SemanticPath}: formed axial endpoint {t.AxialEndpointResidual:G6} mm, {t.Status}");stdout.WriteLine($"Flat: {comparison.FlatPattern.Status}; width Δ {comparison.FlatPattern.WidthResidual:G6}, height Δ {comparison.FlatPattern.HeightResidual:G6}, contour p95 {comparison.FlatPattern.Contour.P95:G6} mm");}
        return comparison.Status==SheetMetalComparisonStatus.Fail?2:0;
    }
    ...
    private static int RunModuleShowcase(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        var galleryReports=new List<object>();var galleryStarted=Stopwatch.GetTimestamp();
        foreach(var entry in SurfacingGallery.Build())
        {
            var brep=SurfacePatchBrepMaterializer.Materialize(entry.Support,entry.Evaluate);if(brep.Body is null){stderr.WriteLine(string.Join("; ",brep.Diagnostics.Select(d=>d.Message)));return 1;}var step=Step242Exporter.ExportBody(brep.Body);if(!step.IsSuccess){stderr.WriteLine(string.Join("; ",step.Diagnostics.Select(d=>d.Message)));return 1;}
            var fileName=entry.StableId+".step";File.WriteAllText(Path.Combine(output,fileName),step.Value);var reimport=Step242Importer.ImportBody(step.Value);
            var authored=entry.Panel.AuthoredPatch;var centerU=(authored.Domain.U.Minimum+authored.Domain.U.Maximum)/2d;var centerV=(authored.Domain.V.Minimum+authored.Domain.V.Maximum)/2d;var curvature=CurvatureQuery.Patch(authored,centerU,centerV);
            galleryReports.Add(new{entry.StableId,panelId=entry.Panel.StableId,constructionFamily=entry.ConstructionKind.ToString(),supportType=entry.Support.Kind.ToString(),materialization=entry.MaterializationKind.ToString(),parameterDomain=new{u=new[]{entry.Panel.ParameterDomain.U.Minimum,entry.Panel.ParameterDomain.U.Maximum},v=new[]{entry.Panel.ParameterDomain.V.Minimum,entry.Panel.ParameterDomain.V.Maximum}},orientation=entry.Panel.Orientation,secondJetCapability=authored.SupportsSecondJet,centerDifferential=new{status=curvature.Status.ToString(),regular=curvature.Singularity==DifferentialSingularityKind.Regular,curvature.GaussianCurvature,curvature.MeanCurvature,curvature.K1,curvature.K2,evidence=curvature.Evidence.ToString(),curvature.Diagnostic},edges=entry.Panel.BoundaryEdges.Select(edge=>new{edge.Name,edge.StableId,curveFamily=edge.Curve.Kind.ToString(),lengthMm=edge.Length(),direction=edge.SourceDirection.ToString(),edge.SourceCurveStableId}),corners=entry.Panel.Corners.Values.Select(corner=>new{corner.Name,corner.StableId,corner.Point}),entry.SourceDeclarations,entry.AuthoringSummary,developability=entry.Developability,approximation=entry.Approximation,step=fileName,stepSha256=Hash(step.Value),brepFaces=brep.Body.Topology.Faces.Count(),stepReimport=reimport.IsSuccess,surfaceMeshIr=entry.Support.Kind==Aetheris.Kernel.Core.Geometry.SurfaceGeometryKind.BSplineSurfaceWithKnots?"typed-unsupported-non-rational-bspline-support":"supported-when-analytic-family-admitted"});
        }
        var galleryMs=Stopwatch.GetElapsedTime(galleryStarted).TotalMilliseconds;
        var canonicalPanelSource="""
Model CanonicalPanel {
  Units: mm;
  Panel Saddle {
    Surface: ParametricSurface {
      DomainU: [-1, 1]; DomainV: [-1, 1];
      X: 40mm * u; Y: 30mm * v; Z: 12mm * u * v;
    }
    Orientation: Front; Thickness: 1.2mm; Material: "Aluminum";
  }
}
""";
        started=Stopwatch.GetTimestamp();var compilerProof=FirmamentPanelCompiler.Compile(canonicalPanelSource);var parseBindMs=compilerProof.Performance.ParseBindMilliseconds;var panelConstructionMs=compilerProof.Performance.PanelConstructionMilliseconds;
        if(!compilerProof.IsSuccess){stderr.WriteLine(string.Join("; ",compilerProof.Diagnostics));return 1;}var compilerProofAgain=FirmamentPanelCompiler.Compile(canonicalPanelSource);
        started=Stopwatch.GetTimestamp();var edgeInspection=compilerProof.Panels.SelectMany(panel=>panel.BoundaryEdges).Select(edge=>new{edge.StableId,length=edge.Length()}).ToArray();var edgeExtractionMs=Stopwatch.GetElapsedTime(started).TotalMilliseconds;
        started=Stopwatch.GetTimestamp();var canopy=PanelShowcases.DevelopableFoldedCanopy();var mateValidationMs=Stopwatch.GetElapsedTime(started).TotalMilliseconds;if(!canopy.Network.IsSuccess){stderr.WriteLine(string.Join("; ",canopy.Network.Diagnostics.Select(d=>d.Message)));return 1;}
        var canopySteps=new List<object>();var stepStarted=Stopwatch.GetTimestamp();for(var i=0;i<canopy.Panels.Count;i++){var panel=canopy.Panels[i];var exportBody=SurfacePatchBrepMaterializer.Materialize(panel.SurfaceConstruction.Support,panel.SurfaceConstruction.Evaluate,panel.Thickness??1);if(exportBody.Body is null){stderr.WriteLine(string.Join("; ",exportBody.Diagnostics.Select(d=>d.Message)));return 1;}var export=Step242Exporter.ExportBody(exportBody.Body);if(!export.IsSuccess){stderr.WriteLine(string.Join("; ",export.Diagnostics.Select(d=>d.Message)));return 1;}var name=$"developable-folded-canopy-{i}.step";File.WriteAllText(Path.Combine(output,name),export.Value);canopySteps.Add(new{panel=panel.StableId,file=name,sha256=Hash(export.Value)});}var stepGenerationMs=Stopwatch.GetElapsedTime(stepStarted).TotalMilliseconds;
        var networkReport=new{canopy.StableId,panels=canopy.Panels.Select(panel=>new{panel.StableId,construction=panel.SurfaceConstruction.Kind.ToString(),developability=panel.Developability,edges=panel.BoundaryEdges.Select(edge=>new{edge.Name,edge.StableId})}),mates=canopy.Network.Mates,freeEdges=canopy.Network.FreeEdges,steps=canopySteps};
        File.WriteAllText(Path.Combine(output,"canonical-panel.firmament"),canonicalPanelSource);File.WriteAllText(Path.Combine(output,"panel-network.json"),JsonSerializer.Serialize(networkReport,JsonOptions));File.WriteAllText(Path.Combine(output,"compiler-bridge.json"),JsonSerializer.Serialize(new{success=compilerProof.IsSuccess,panels=compilerProof.Panels.Select(panel=>panel.StableId),deterministic=compilerProof.Panels.Select(panel=>panel.StableId).SequenceEqual(compilerProofAgain.Panels.Select(panel=>panel.StableId)),compilerProof.TemplateInstantiations},JsonOptions));
        var evidence=new{milestone="AETHERIS-PANEL-M0",registryMs,cachedRegistryLookupMicroseconds,surfacingMs,galleryMs,pipingMs,performance=new{parseBindMs,panelConstructionMs,edgeExtractionMs,mateValidationMs,stepGenerationMs},determinism=new{panelIds=compilerProof.Panels.Select(panel=>panel.StableId),repeatPanelIds=compilerProofAgain.Panels.Select(panel=>panel.StableId),stable=compilerProof.Panels.Select(panel=>panel.StableId).SequenceEqual(compilerProofAgain.Panels.Select(panel=>panel.StableId))},modules=catalog.Modules.Select(m=>new{module=m.Id.Value,version=m.Version.ToString(),capabilities=m.Capabilities.Select(c=>c.Id)}),surfacing=new{saddleIr.StableId,kind=saddleIr.Kind.ToString(),surface=saddlePatch.Patch!.ExactSurface.Kind.ToString(),boundaries=saddlePatch.Patch.BoundaryProvenance,developability=saddlePatch.Patch.Developability,saddleHash=Hash(saddleStep.Value),faces=saddlePanel.Body.Topology.Faces.Count(),gallery=galleryReports},panels=new{compilerBridge=compilerProof.Panels.Select(panel=>new{panel.StableId,panel.Orientation,panel.Developability,semantics=SemanticValueDescriptor.From(panel.SemanticValue)}),edgeInspection,network=networkReport},piping=new{ir=new{pipe.Ir!.StableId,elements=pipe.Ir.Elements.Select(e=>e switch{PipeRouteElement.Straight s=>new{kind="Straight",start=s.Start,end=s.End,radius=(double?)null,angle=(double?)null},PipeRouteElement.PlanarCircularBend b=>new{kind="PlanarCircularBend",start=b.Center,end=b.Center,radius=(double?)b.Radius,angle=(double?)b.AngleRadians},_=>throw new InvalidOperationException()})},framePolicy=PipeRouteLowering.CircularFramePolicy,pipeHash=Hash(pipeStep.Value),faces=pipe.Body!.Topology.Faces.Count(),semantics=SemanticValueDescriptor.From(pipe.Semantics!)} };
        ...
        File.WriteAllText(Path.Combine(output,"validation-report.json"),JsonSerializer.Serialize(evidence,JsonOptions));File.WriteAllText(Path.Combine(output,"surfacing-gallery-inspection.json"),JsonSerializer.Serialize(galleryReports,JsonOptions));
        var hashManifest=Directory.GetFiles(output).Where(path=>Path.GetFileName(path)!="hashes.json").Order(StringComparer.Ordinal).Select(path=>new{file=Path.GetFileName(path),sha256=Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(path)))}).ToArray();File.WriteAllText(Path.Combine(output,"hashes.json"),JsonSerializer.Serialize(hashManifest,JsonOptions));
        stdout.WriteLine(JsonSerializer.Serialize(new{output,saddleStep=saddlePath,pipeStep=pipePath,saddleHash=Hash(saddleStep.Value),pipeHash=Hash(pipeStep.Value),surfacingGallery=galleryReports.Count,panels=galleryReports.Count,networkPanels=canopy.Panels.Count,networkMates=canopy.Mates.Count},JsonOptions));return 0;
    }
    ...
    private static int RunFea(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        {
            var message = $"Analysis source was not found: {input}";
            if (json) stdout.WriteLine(JsonSerializer.Serialize(new { command = "fea", success = false, input, diagnostics = new[] { new { code = "firmament-analysis-source-missing", severity = "Error", message } } }, JsonOptions));
        ...
        if (!compiled.IsSuccess || compiled.Analysis is null)
        {
            if (json)
                stdout.WriteLine(JsonSerializer.Serialize(new
                {
                    ...
                    success = false,
                    input,
                    diagnostics = compiled.Diagnostics.Select(diagnostic => new { diagnostic.Code, severity = diagnostic.Severity.ToString(), diagnostic.Message })
                }, JsonOptions));
            else
        ...
        if (WireFormAuthoring.IsWireFormSource(source))
        {
            ...
            var wireReportPayload = new
            {
                ...
                wireForm = wire is null ? null : new
                {
                    ...
                    operationCount = wire.Operations.Count, straightCount = wire.Operations.Count(x => x is WireStraightAir), bendCount = wire.Operations.Count(x => x is WireBendAir), coilCount = wire.Operations.Count(x => x is WireCoilAir), knotCount = wire.Operations.Count(x => x is WireKnotPathAir),
                    ...
                    operations = wire.Operations.Select(operation => new { operation.Ordinal, operation.Name, kind = operation switch { WireStraightAir => "Straight", WireBendAir => "Bend", WireAxisCoilAir => "AxisCoil", WireSurfaceCoilAir => "SurfaceCoil", WireKnotPathAir => "KnotPath", _ => "Unknown" }, operation.LengthMm,
                        ...
                        supportClearance = operation is WireCoilAir support ? support.SupportClearanceMm : null, representation = operation switch { WireKnotPathAir => "ExactEvaluablePeriodicKnotLaw->ClosureCorrectedNonRationalBSplineTube", WireCoilAir => "ExactEvaluableWindingLaw->NonRationalBSplineTube", _ => "Analytic" },
                        approximationTolerance = operation is WireEvaluablePathAir tolerance ? tolerance.ApproximationToleranceMm : (double?)null,
                        approximationSegmentCount = operation is WireEvaluablePathAir segments ? segments.ApproximationSegmentCount : (int?)null,
                        approximationMaxError = operation is WireEvaluablePathAir maxError ? maxError.ApproximationError.MaxMm : (double?)null,
                        approximationRmsError = operation is WireEvaluablePathAir rmsError ? rmsError.ApproximationError.RmsMm : (double?)null,
                        knot = operation is WireKnotPathAir knot ? new { family = knot.Family.ToString(), knot.P, knot.Q, knot.ComponentCount, knot.Closed, knot.ScaleMm, knot.MajorRadiusMm, knot.MinorRadiusMm, handedness = knot.Handedness.ToString(), phaseDegrees = knot.PhaseRadians * 180d / Math.PI, seamParameter = knot.SeamParameter,
                            centerlineLength = knot.LengthMm, lengthIntegrationError = knot.Qualification.LengthIntegrationErrorMm, minimumNonlocalCenterlineDistance = knot.Qualification.MinimumNonlocalDistanceMm, minimumLocalCurvatureRadius = knot.Qualification.MinimumCurvatureRadiusMm, tubeRadiusLimit = knot.Qualification.TubeRadiusLimitMm, maximumAdmittedDiameter = knot.Qualification.MaximumAdmittedDiameterMm, closestParameter1 = knot.Qualification.ClosestParameter1, closestParameter2 = knot.Qualification.ClosestParameter2,
                            rawClosureRotation = knot.FrameClosure.RawClosureRotationRadians, appliedDistributedCorrection = knot.FrameClosure.AppliedCorrectionRadians, finalClosureRotationError = knot.FrameClosure.FinalClosureErrorRadians, qualificationMethod = knot.Qualification.Method } : null,
    ...
    private static int RunInspectCompose(string[] args, TextWriter stdout, TextWriter stderr)
    {
        ...
        var report = new
        {
            composition = new
            {
                ...
                shaftHoles = (stack.Feature.ShaftHoles ?? []).Select(x => new { x.Name, x.StableId, profile = x.ProfileReference, center = new[] { x.CenterX, x.CenterY }, x.Diameter, x.From, x.To, endCondition = "ThroughAll", x.SemanticRole, x.SourceSpan, support = x.SupportSpan is null ? null : new { kind = "Span<Plane>", id = x.SupportSpan, parent = x.ParentSupport, boundaryMargin = x.SupportBoundaryMargin } }),
                ...
                bosses = (stack.Feature.Bosses ?? []).Select(x => new { x.Name, x.StableId, host = x.Host, on = x.SupportFace, profile = x.ProfileReference, x.Height, x.From, x.To, support = x.SupportSpan is null ? null : new { kind = "Span<Plane>", id = x.SupportSpan, parent = x.ParentSupport }, materialEffect = "Add", lowering = "PrismaticSectionStack/Add" }),
                pockets = (stack.Feature.Pockets ?? []).Select(x => new { x.Name, x.StableId, host = x.Host, on = x.SupportFace, profile = x.ProfileReference, x.Depth, x.HostThickness, x.RemainingFloor, x.MinimumFloorThickness, x.MinimumFloorPolicySource, x.From, x.To, support = x.SupportSpan is null ? null : new { kind = "Span<Plane>", id = x.SupportSpan, parent = x.ParentSupport }, materialEffect = "Remove", lowering = "PrismaticSectionStack/Remove" }),
    ...
    private static void WriteSummaryText(AnalyzeResult analysis, TextWriter stdout)
    {
        ...
        if (analysis.SemanticPmi is { } semanticPmi)
        {
            stdout.WriteLine($"Semantic PMI: {semanticPmi.Items.Count} records (datums={semanticPmi.DatumCount}, dimensions={semanticPmi.DimensionCount}, geometricTolerances={semanticPmi.GeometricToleranceCount}, annotations={semanticPmi.AnnotationCount})");
            foreach (var item in semanticPmi.Items)
                stdout.WriteLine($"  {item.Kind} {item.Name}: target={item.Target}, value={FormatDouble(item.Value)}, quantity={item.Quantity?.ToString() ?? "n/a"}, datums=[{string.Join(",", item.DatumReferences)}]");
        }
    ...
    private static void WriteVerifyHelp(TextWriter stdout)
    {
        ...
        stdout.WriteLine("The report is hash-tied to artifacts/local/verification/<fixture>/<sha256>/ by default.");
        stdout.WriteLine();
        stdout.WriteLine("Options:");
