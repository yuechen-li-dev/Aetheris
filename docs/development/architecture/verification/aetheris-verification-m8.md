...
`aetheris verify model.step` is the artifact-level verifier. It writes a hash-tied report under `artifacts/local/verification/<fixture>/<step-sha256>/` by default, including a copied STEP artifact and `verification-report.json`.
