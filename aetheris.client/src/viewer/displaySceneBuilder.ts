...
export function buildDisplaySceneData(
  ...
  const fallbackDisplayScene = !sceneHasTypedRenderable(typedDisplayScene) && preparation.tessellationFallback
    ? legacyTessellationToDisplayScene(preparation.tessellationFallback)
    : null;
  const primaryDisplayScene = sceneHasTypedRenderable(typedDisplayScene) ? typedDisplayScene : fallbackDisplayScene;
  ...
  if (preparation.lane === 'mixed-fallback') {
    ...
    const renderedFallbackFaceIds = new Set(
      (primaryDisplayScene?.renderables ?? [])
        .filter((renderable) => renderable.kind === 'MeshPatch' && fallbackFaceIds.has(renderable.faceId))
