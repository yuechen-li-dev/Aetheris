...
public sealed record PolicyShapeScenario(string Id, string Group, string Description, SdfNode? Root, string Category, bool Deferred = false);
...
public static class FrepPolicyShapeDoeLab
{
    ...
    [
        new("A1","A","box - centered cylinder through Z", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfCylinderNode(2,8)), "canonical_through_hole"),
        new("A2","A","box - offset cylinder through Z with strict clearance", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfTransformNode(new SdfCylinderNode(1.5,8), Transform3D.CreateTranslation(new(1,1,0)))), "canonical_through_hole"),
        new("A3","A","translated box - translated cylinder through Z", new SdfSubtractNode(new SdfTransformNode(new SdfBoxNode(10,8,6), Transform3D.CreateTranslation(new(3,2,5))), new SdfTransformNode(new SdfCylinderNode(2,8), Transform3D.CreateTranslation(new(3.5,2.5,5)))), "canonical_through_hole"),
        new("A4","A","rectangular plate - cylinder through Z", new SdfSubtractNode(new SdfBoxNode(12,8,2), new SdfCylinderNode(1,3)), "canonical_through_hole"),
        new("B1","B","box - blind short cylinder", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfCylinderNode(2,4)), "invalid_hole"),
        new("B2","B","box - tangent/grazing cylinder", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfTransformNode(new SdfCylinderNode(2,8), Transform3D.CreateTranslation(new(3,0,0)))), "invalid_hole"),
        new("B3","B","box - cylinder outside box", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfTransformNode(new SdfCylinderNode(2,8), Transform3D.CreateTranslation(new(7,0,0)))), "invalid_hole"),
        new("B4","B","box - oversized cylinder", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfCylinderNode(6,8)), "invalid_hole"),
        new("B5","B","box - rotated cylinder", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfTransformNode(new SdfCylinderNode(2,8), Transform3D.CreateRotationX(Math.PI/4))), "invalid_hole"),
        new("C1","C","box - cylinder side notch", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfTransformNode(new SdfCylinderNode(2,8), Transform3D.CreateTranslation(new(4,0,0)))), "cylindrical_non_through"),
        new("C2","C","box - shallow cylindrical pocket", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfCylinderNode(1.5,1.5)), "cylindrical_non_through"),
        ...
        new("F2","F","box - sphere", new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfSphereNode(2)), "unsupported"),
        new("F3","F","nested booleans", new SdfSubtractNode(new SdfSubtractNode(new SdfBoxNode(10,8,6), new SdfCylinderNode(1,8)), new SdfCylinderNode(1,8)), "unsupported"),
        new("F4","F","union/intersect cases", new SdfUnionNode(new SdfBoxNode(1,1,1), new SdfCylinderNode(1,1)), "unsupported")
    ];
    ...
    private sealed class CylindricalToolCut : IPolicyShapeCandidate { public string Name => "CylindricalToolCutPolicyShape"; public int TieBreakerPriority=>1; public PolicyShapeEvaluation Evaluate(PolicyShapeScenario s){ var toolCyl = s.Root is SdfSubtractNode sub && (sub.Right is SdfCylinderNode || sub.Right is SdfTransformNode { Child: SdfCylinderNode }); var over = toolCyl && s.Category=="cylindrical_non_through"; return new(Name, toolCyl, toolCyl?72:0, toolCyl?PolicyShapeCapability.DiagnosticOnly:PolicyShapeCapability.Unsupported, RecoveryPlanShape.CylindricalToolCutPlan, toolCyl?["tool-family=cylindrical"]:[], toolCyl?[]:["tool is not cylindrical subtract"], over, false, 5, 5, over?["over-admission-risk: mixes hole/notch/pocket/groove"]:[]);} }
    private sealed class GenericBoolean : IPolicyShapeCandidate { public string Name => "CirBooleanRecoveryPolicyShape"; public int TieBreakerPriority=>2; public PolicyShapeEvaluation Evaluate(PolicyShapeScenario s){ var admissible = s.Root is SdfSubtractNode or SdfUnionNode or SdfIntersectNode; return new(Name, admissible, admissible?40:0, admissible?PolicyShapeCapability.DiagnosticOnly:PolicyShapeCapability.Unsupported, RecoveryPlanShape.GenericBooleanRecoveryPlan, admissible?["generic-boolean"]:[], admissible?[]:["not a boolean CIR root"], admissible, false, 8, 2, admissible?["dispatcher-risk: broad boolean bucket"]:[]);} }
