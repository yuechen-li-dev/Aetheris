...
public sealed class ConceptStepMatchTests
{
    ...
    public void Match_StandardBracket_ReportsTypedAnalyticEvidence()
    {
        ...
        var exit = CliRunner.Run(["match", step, Path.Combine(root, "fixtures", "Regression", "DemoRegression", "concept-step-match-standard.firmament"), "--json"], stdout, stderr);
        Assert.Equal(0, exit);
    ...
    {
        var root = RepositoryRoot(); var source = File.ReadAllText(Path.Combine(root, "fixtures", "Regression", "DemoRegression", "concept-step-match-standard.firmament"));
    ...
    private static string BuildStandardStep(string root)
    {
        ...
        Assert.Equal(0, CliRunner.Run(["build", Path.Combine(root, "fixtures", "Regression", "DemoRegression", "concept-step-match-standard.firmament"), "--out", step, "--json"], output, errors));
        return step;
    }
}
