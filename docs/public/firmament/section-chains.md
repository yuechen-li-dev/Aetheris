...
The compatibility command also supports the generated `flagship`, `twist`, and `two-profile` witnesses. A build writes sibling `.evidence.json` containing section frames, spans, correspondence, transition classes, terminations, topology, pcurve qualification, conservative intersection evidence, timings, SHA-256, rational-surface count, and STEP reimport evidence.

`AddSectionChain` and `RemoveSectionChain` are not yet qualified BodyState operations. The current kernel's bounded Boolean families do not admit arbitrary changing ruled/B-spline tools, and the existing housing state rebuild would erase an attached chain during a downstream feature. A future admitted lane must construct a single shared-topology shell, retain chain history, enforce an authorized region and preservation contracts, and fail transactionally. These operations must not be represented publicly as arbitrary Boolean authoring.

## Current limits

The admitted lane has one closed outer loop, no holes, same-topology one-to-one correspondence, ruled transitions, and Cap/Open terminals. It checks frame handedness, loop closure and orientation, sampled profile self-intersection, section spacing, semantic mapping order, sampled transition Jacobian foldover, non-neighbour transition crossings, and remote cap penetration using a deterministic validation-only triangle proxy. The proxy is conservative detection evidence, not a global proof. It does not claim general smooth lofting, rails, topology-changing correspondence, globally proven intersection freedom, or production arbitrary sculpt attachment/subtraction. Invalid chains fail closed rather than producing a faceted product fallback.
