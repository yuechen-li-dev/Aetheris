...
public sealed record TrianglePrismChamferAdjacencyParityRow(string CaseName, bool LegacyProduced, bool CandidateProduced, TrianglePrismAdjacencyGraphSummary LegacyAdjacency, TrianglePrismAdjacencyGraphSummary CandidateAdjacency, TrianglePrismChamferCandidateSummary LegacyChamfer, TrianglePrismChamferCandidateSummary CandidateChamfer, bool TopologySummaryParity, bool AdjacencySummaryParity, bool FeatureRecognitionParity, string? FirstDivergence, string BlockerClassification, IReadOnlyList<string> Diagnostics, string Recommendation);
...
public static class TrianglePrismChamferAdjacencyParityLab
{
    ...
    public static TrianglePrismChamferAdjacencyParityRow Run(string caseName, double width, double depth, double height, double chamferDistance)
    {
        ...
        if (legacyBody is null || candidateBody is null)
        {
            ...
            return new(caseName, legacyBody is not null, candidateBody is not null, EmptyAdjacency(), EmptyAdjacency(), EmptyChamfer(), EmptyChamfer(), false, false, false, "body-production-failed", "candidate-construction-bug", diagnostics, "triangle-feature-recognition-keep-legacy-route");
        }
        ...
        var topologyParity = legacyAdj.FaceCount == candidateAdj.FaceCount
            && legacyAdj.EdgeCount == candidateAdj.EdgeCount
            && legacyAdj.VertexCount == candidateAdj.VertexCount
            && legacyAdj.PlanarFaceCount == candidateAdj.PlanarFaceCount;
        var adjacencyParity = legacyAdj.EdgeRows.SequenceEqual(candidateAdj.EdgeRows, StringComparer.Ordinal)
            && legacyAdj.VertexRows.SequenceEqual(candidateAdj.VertexRows, StringComparer.Ordinal);
        ...
        var blockerClassification = parity ? "none" : recommendation.Replace("triangle-feature-recognition-needs-", string.Empty, StringComparison.Ordinal).Replace("triangle-feature-recognition-", string.Empty, StringComparison.Ordinal);
        if (!parity)
            diagnostics.Add($"v2-x8-1-blocker-classified:{blockerClassification}");

        return new(caseName, true, true, legacyAdj, candidateAdj, legacyChamfer, candidateChamfer, topologyParity, adjacencyParity, parity, firstDivergence, blockerClassification, diagnostics.Distinct().OrderBy(x => x, StringComparer.Ordinal).ToArray(), recommendation);
    }
