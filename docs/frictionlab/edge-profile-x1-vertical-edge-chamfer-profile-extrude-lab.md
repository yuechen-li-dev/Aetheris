...
`docs/edge-a2-constructive-chamfer-reframing-audit.md` classifies vertical extrusion-edge chamfers as profile modifications: the sharp edge should not be emitted and then trimmed. Instead, the rectangle corner is replaced by a bevel segment in 2D, and the final pentagonal profile is extruded directly.
