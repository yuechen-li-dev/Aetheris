...
export function legacyFacePatchToMeshRenderable(
	patch: TessellationResponseDto["facePatches"][number],
): MeshRenderable {
	return {
		kind: "MeshPatch",
		faceId: patch.faceId,
...
export function legacyTessellationToDisplayScene(
	tessellation: TessellationResponseDto,
): DisplayScene {
	const meshRenderables: DisplayRenderable[] = tessellation.facePatches.map(
		legacyFacePatchToMeshRenderable,
	);
