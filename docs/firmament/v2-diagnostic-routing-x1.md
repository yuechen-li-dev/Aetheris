...
`fixtures/FirmamentV2/Language/invalid/concept-struct-diagnostic-routing-x1.invalid.firmfixture` is a minimized Concept/Struct document with a malformed construction-plane `Hole<Shaft>` center and an omitted required Concept member. Both `validate` and `build` surface `HoleLocalCenterInvalid` and `firmament-concept-missing-member:BracketConcept.RequiredExpose`; build does not surface `FIRM-PARSE-0001` or the canonical TOON/JSON-root wording.
