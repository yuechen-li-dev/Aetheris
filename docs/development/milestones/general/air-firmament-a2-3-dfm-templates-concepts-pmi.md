...
A2.3 adds metadata-only Firmament V2 fixtures under `fixtures/Templates/` and `fixtures/PMI/`. Because no V2 parser exists, valid examples are classified as future design / parser-not-ready / not-implemented by metadata, invalid examples report metadata diagnostics, and none are sent through the V1 parser as random parse failures.
