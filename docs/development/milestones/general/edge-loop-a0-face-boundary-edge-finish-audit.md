...
### EDGE-LOOP-X1 evidence note

`PrismaticTopFaceLoopChamferPrototype` and `TopFaceLoopChamferPrismaticLab` now realize this first-scope case. The canonical row uses width `10`, depth `8`, height `6`, and chamfer distance `1`; validates the top-cap outer closed four-edge loop as a Class B selection; emits sections at `z0 = 0`, `z1 = 5`, and `z2 = 6`; and routes through `PrismaticSectionTransitionEmitter`. The split-preserving topology is `12` vertices, `20` edges, `10` all-planar faces, `10` loops, `40` coedges, and bounds `[-5,-4,0]..[5,4,6]`. STEP smoke confirms planar manifold BRep markers and no cylindrical surfaces or void BReps. Diagnostics explicitly state that this is one Class B loop route, not four independent Class A single-edge chamfers, and that it uses no AirEdgeSweep, no BrepBoundedChamfer, no topology graft, no 3D Boolean, and no coplanar merge.
