...
public sealed class SheetMetalM8Tests
{
    ...
    public void Ctc03_FinalSemanticLayout_LowersAllOpeningsAndPartialTabFlangeWithoutSourceStep()
    {
        ...
        try
        {
            ...
            Assert.Equal(9,result.Spec!.SemanticLayout.Patterns.Count);Assert.Single(result.Spec.SemanticLayout.Tabs);Assert.Equal(2,result.Spec.SemanticLayout.SteppedNotches!.Count);Assert.Equal(10,result.Spec.SemanticLayout.Corners!.Count);
