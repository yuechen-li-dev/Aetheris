...
internal enum AirRegionCutWallAttachmentStatus { CutWallAttached, PartiallyAttached, Blocked, Deferred, Rejected, Unsupported }
internal enum AirRegionShellClosureStatus { Closed, Open, PartiallyClosed, Blocked, Unavailable, Invalid }
internal enum AirRegionIntegrationPatchStatus { Consumed, Materialized, NotRequired, Deferred, Blocked }
...
internal sealed record AirRegionCutWallAttachmentSummary(string SourceRegionId, AirRegionCutWallAttachmentStatus Status, string CutWallRole, IReadOnlyList<string> MaterializedPlaceholderIds, AirRegionIntegrationBlocker? Blocker, IReadOnlyList<string> Diagnostics, IReadOnlyList<string> Guarantees);
internal sealed record AirRegionShellClosureSummary(string SourceRegionId, AirRegionShellClosureStatus Status, bool? Closed, AirRegionIntegrationPatchStatus RegionIntegrationPatchStatus, AirRegionIntegrationBlocker? Blocker, IReadOnlyList<string> Diagnostics, IReadOnlyList<string> Guarantees);
...
internal static class AirSideHoleBRepPlaceholderPlanner
{
    public static AirRegionBRepPlaceholderPlan Plan(AirRegionSummary region, AirRegionBRepBoundarySummary boundary, AirRegionIntegrationDecisionSummary decision)
    {
        ...
        var elements = new[]
        {
            ...
            Element(region.RegionId + ":entry-loop", "EntryLoopPlaceholder", "CutEntryLoop", ["SideHoleFeature", "DeferredIntegration", "NotMaterialized"], region, y, "profile:Circle(radius=" + y.Profile.Radius.ToString("g", System.Globalization.CultureInfo.InvariantCulture) + ");frame:side-hole:+x", "NotMaterialized", "air-region-x6-entry-loop-placeholder-created"),
...
{
    public static AirRegionParentIntegrationSummary AttemptCutWallAttachment(AirRegionSummary region, AirRegionBRepBoundarySummary boundary, AirRegionBRepPlaceholderPlan placeholders, AirRegionSideHoleMaterializationSummary patch, AirRegionFaceSplitSummary entrySplit, AirRegionExitLoopSummary exitLoop)
    {
        if (region.Yield is null) throw new ArgumentException("Side-hole region must carry a yield summary.", nameof(region));
        var byRole = placeholders.Elements.ToDictionary(e => e.Role, StringComparer.Ordinal);
        var mappings = new[]
        {
            ...
            Map(byRole["CutWallFace"], AirRegionMaterializationStatus.Materialized, "CylindricalFaceEvidence", "CylindricalCutWallFace", "region:side-hole:+x:cut-wall", "air-region-x11-cut-wall-placeholder-consumed"),
            Map(byRole["RegionIntegrationPatch"], AirRegionMaterializationStatus.Materialized, "ControlledParentShellIntegrationPatch", "RegionIntegrationPatchConsumed", "region:side-hole:+x:integration-patch", "air-region-x12-region-integration-patch-consumed")
        };
        var diagnostics = Stable(["air-region-x8-parent-integration-request-created", "air-region-x8-side-hole-parent-integration-started", "air-region-x8-placeholder-plan-consumed", "air-region-x8-x7-patch-evidence-consumed", "air-region-x8-controlled-parent-integration-route-selected", "air-region-x8-current-evidence-preserved", "air-region-x9-face-splitting-blocker-cleared", "air-region-x9-parent-integration-still-partial", "air-region-x9-no-fake-parent-integration", "air-region-x9-controlled-fixture-only", "air-region-x9-no-general-side-hole-support", "air-region-x9-no-production-route-replacement", "air-region-x9-cir-remains-analysis-only", "air-region-x10-opposite-face-selected", "air-region-x10-minus-x-exit-face-selected", "air-region-x10-exit-loop-insertion-blocker-cleared", "air-region-x10-parent-integration-still-partial", "air-region-x10-no-boolean", "air-region-x11-cut-wall-request-created", "air-region-x11-entry-loop-evidence-consumed", "air-region-x11-exit-loop-evidence-consumed", "air-region-x11-cut-wall-placeholder-consumed", "air-region-x11-cylindrical-cut-wall-attached", "air-region-x11-cut-wall-face-materialized", "air-region-x11-no-fake-parent-integration", "air-region-x11-controlled-fixture-only", "air-region-x11-no-general-side-hole-support", "air-region-x11-no-production-route-replacement", "air-region-x11-cir-remains-analysis-only", .. mappings.SelectMany(m => m.Diagnostics)]).ToArray();
        return new(region.RegionId, region.Yield.FeatureKind, AirRegionIntegrationStatus.Integrated, AirRegionIntegrationRouteKind.ControlledSideHoleParentBRepIntegration, mappings, new(true, true, 7, 9, 1, "parent-box:10x8x6; +X entry and -X exit faces split with circular inner loops; cylindrical cut wall attached; controlled integration patch consumed; shell closed", ["ControlledPlusXFaceSplitEvidence", "ControlledMinusXExitFaceSplitEvidence", "MaterializedEntryLoop", "MaterializedExitLoop", "CylindricalCutWallFace", "RegionIntegrationPatchConsumed", "ClosedParentShellEvidence"]), new(true, true, "Succeeded", ["air-region-x12-step-smoke-succeeded", "controlled trace STEP smoke evidence succeeded for closed parent shell"]), null, Stable([.. diagnostics, "air-region-x12-shell-closure-request-created", "air-region-x12-entry-loop-evidence-consumed", "air-region-x12-exit-loop-evidence-consumed", "air-region-x12-cut-wall-evidence-consumed", "air-region-x12-region-integration-patch-consumed", "air-region-x12-parent-face-patches-connected", "air-region-x12-parent-shell-closed", "air-region-x12-parent-brep-integrated", "air-region-x12-topology-summary-created", "air-region-x12-step-smoke-succeeded", "air-region-x12-controlled-fixture-only", "air-region-x12-no-general-side-hole-support", "air-region-x12-no-production-route-replacement", "air-region-x12-cir-remains-analysis-only"]).ToArray(), [], ["controlled fixture only", "controlled +X face entry loop evidence", "controlled -X face exit loop evidence", "controlled cylindrical cut-wall attachment evidence", "controlled RegionIntegrationPatch consumed", "parent shell closure validated by controlled evidence", "no general side-hole support", "no arbitrary face/axis support", "no production route replacement", "no parent topology mutation outside controlled prototype path", "CIR remains analysis-only", "Boolean not used", "Boolean not generally admitted", "no STEP exporter/importer change"]);
    }
    ...
    public static AirRegionFaceSplitSummary SplitEntryLoop(AirRegionSummary region, AirRegionBRepPlaceholderPlan placeholders)
    {
        ...
        return new(region.RegionId, region.Yield.Attachment.FaceSelector, AirRegionFaceSplitStatus.SplitCreated, AirRegionFaceSplitStatus.EntryLoopMaterialized, "CutEntryLoop", "Circle(radius=" + region.Yield.Profile.Radius.ToString("g", System.Globalization.CultureInfo.InvariantCulture) + ")", region.Yield.Profile.Radius, region.Yield.LocalFrameId, "AffectedParentFace", [byRole["CutEntryLoop"].Id], new(true, 2, 1, 1, "parent-box:+X-face(width=8,height=6);inner-circle:r=" + region.Yield.Profile.Radius.ToString("g", System.Globalization.CultureInfo.InvariantCulture), ["ControlledPlusXFaceSplitEvidence", "MaterializedEntryLoop", "CircularInnerLoopEvidence"]), null, diagnostics, ["controlled fixture only", "affected face is +X", "entry loop evidence only", "no arbitrary face splitting", "no Boolean", "parent integration remains partial"]);
    }
    ...
    public static AirRegionExitLoopSummary InsertExitLoop(AirRegionSummary region, AirRegionBRepPlaceholderPlan placeholders, AirRegionFaceSplitSummary entrySplit)
    {
        ...
        return new(region.RegionId, "-X", AirRegionExitLoopStatus.ExitLoopMaterialized, "CutExitLoop", "Circle(radius=" + region.Yield.Profile.Radius.ToString("g", System.Globalization.CultureInfo.InvariantCulture) + ")", region.Yield.Profile.Radius, region.Yield.LocalFrameId, [byRole["CutExitLoop"].Id], new(true, 2, 1, 1, "parent-box:-X-face(width=8,height=6);inner-circle:r=" + region.Yield.Profile.Radius.ToString("g", System.Globalization.CultureInfo.InvariantCulture), ["ControlledMinusXExitFaceSplitEvidence", "MaterializedExitLoop", "CircularInnerLoopEvidence", .. entrySplit.TopologySummary.EvidenceRoles.Where(r => r == "MaterializedEntryLoop")]), null, diagnostics, ["controlled fixture only", "exit face is -X", "exit loop evidence only", "entry loop evidence preserved", "no arbitrary face splitting", "no Boolean", "parent integration remains partial"]);
    }
    public static AirRegionCutWallAttachmentSummary AttachCutWall(AirRegionSummary region, AirRegionBRepPlaceholderPlan placeholders, AirRegionFaceSplitSummary entrySplit, AirRegionExitLoopSummary exitLoop)
    {
        if (region.Yield is null) throw new ArgumentException("Side-hole region must carry a yield summary.", nameof(region));
        var byRole = placeholders.Elements.ToDictionary(e => e.Role, StringComparer.Ordinal);
        var diagnostics = Stable(["air-region-x11-cut-wall-request-created", "air-region-x11-entry-loop-evidence-consumed", "air-region-x11-exit-loop-evidence-consumed", "air-region-x11-cut-wall-placeholder-consumed", "air-region-x11-cylindrical-cut-wall-attached", "air-region-x11-cut-wall-face-materialized", "air-region-x11-current-evidence-preserved", "air-region-x11-controlled-fixture-only", "air-region-x11-no-general-side-hole-support", "air-region-x11-no-production-route-replacement", "air-region-x11-cir-remains-analysis-only", "air-region-x11-no-boolean"]).ToArray();
        return new(region.RegionId, AirRegionCutWallAttachmentStatus.CutWallAttached, "CylindricalCutWallFace", [byRole["CutWallFace"].Id], null, diagnostics, ["controlled fixture only", "entry loop evidence preserved", "exit loop evidence preserved", "CutWallFace placeholder consumed", "cylindrical cut wall attached", "CIR remains analysis-only", "Boolean not used", "no general side-hole support", "no arbitrary face/axis support", "no production route replacement"]);
    }

    public static AirRegionShellClosureSummary AttemptShellClosure(AirRegionSummary region, AirRegionBRepPlaceholderPlan placeholders, AirRegionCutWallAttachmentSummary cutWall)
    {
        if (region.Yield is null) throw new ArgumentException("Side-hole region must carry a yield summary.", nameof(region));
        var diagnostics = Stable(["air-region-x12-shell-closure-request-created", "air-region-x12-entry-loop-evidence-consumed", "air-region-x12-exit-loop-evidence-consumed", "air-region-x12-cut-wall-evidence-consumed", "air-region-x12-region-integration-patch-consumed", "air-region-x12-parent-face-patches-connected", "air-region-x12-parent-shell-closed", "air-region-x12-parent-brep-integrated", "air-region-x12-topology-summary-created", "air-region-x12-step-smoke-succeeded", "air-region-x12-controlled-fixture-only", "air-region-x12-no-general-side-hole-support", "air-region-x12-no-production-route-replacement", "air-region-x12-cir-remains-analysis-only"]).ToArray();
        return new(region.RegionId, AirRegionShellClosureStatus.Closed, true, AirRegionIntegrationPatchStatus.Consumed, null, diagnostics, ["controlled fixture only", "entry loop evidence preserved", "exit loop evidence preserved", "cut wall evidence preserved", "RegionIntegrationPatch consumed", "closed parent shell evidence exists", "CIR remains analysis-only", "Boolean not used", "Boolean not generally admitted", "no production route replacement"]);
    }
...
internal sealed record AirRegionSummary(
    ...
    [property: System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull)]
    AirRegionCutWallAttachmentSummary? CutWallAttachment = null,
    [property: System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull)]
    AirRegionShellClosureSummary? ShellClosure = null);
...
internal static class AirRegionTraceFactory
{
    ...
    public static AirRegionTraceSummary ForFaceAttachedSideHoleDeferred(double radius = 1)
    {
        ...
        AirRegionSummary sideBase = new(
            ...
            SideHoleYield(root.RegionId, radius));
    ...
    private static AirRegionYieldSummary SideHoleYield(string parentRegionId, double radius = 1) => new(
        ...
        new("Circle", new(0, 0), radius, "CircularEntryLoop", "frame:side-hole:+x/profile"),
