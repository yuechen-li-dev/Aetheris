...
Content search found 333 repository files with a V1 `firmament:` root across `testdata/firmament`, the older FrictionLab case corpus, and one parser-backed `fixtures` fixture. No committed JSON-shaped V1 source fixture was found by the same structural search; JSON coverage lives mainly as inline test strings.
