...
Status note, 2026-06-22: RULED-A1 has now landed the minimal exact swept-surface skeleton for `SURFACE_OF_LINEAR_EXTRUSION` and `SURFACE_OF_REVOLUTION`, plus degree-1 ruled B-spline classification. See `docs/development/implementation/ruled-a1-exact-swept-surface-skeleton.md`.
...
```powershell
pwsh ./tools/Validate-Step-FreeCAD.ps1 ./testdata/step242/golden/firmament-v1/cylinder_basic.step
...
- `docs/development/milestones/general/air-v5-frustum-ruled-transition-production.md` — confirms older `RULED_SURFACE` exporter work was explicitly not in that milestone's scope.
