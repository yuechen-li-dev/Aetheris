...
| Bounded mathematical sculpting | Bounded (SURF-X2 subset) | Immutable single-predecessor `BodyState`; rectangular-housing `OffsetRegion`; direct or imported-face `ReplaceRegion`; qualified crown-to-planar-shoulder `BlendBoundary` with hard G1/G2 evidence, four deterministic polynomial candidates, fairness/curvature/compactness/complexity judgment, override/fallback trace, qualified intersections and pcurves, exact lower-body locality, AP242 association persistence, and rational-free STEP. General blend networks remain unsupported. See the [sculpting guide](../firmament/sculpting.md). |
...
| SectionChain ruled loft substrate | Experimental (SURF-X3a progression) | Firmament Concept Path/Profile authoring lowers into ordered framed, same-topology semantic SectionChain IR; one-to-one ruled transitions; shared internal edges; Cap/Open; strict face-local pcurves; deterministic validation-only non-neighbour/cap intersection detection; exact Plane and non-rational B-spline STEP with reimport. BodyState Add/Remove is not yet qualified. See [Section chains](../firmament/section-chains.md). |
