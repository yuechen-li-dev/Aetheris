...
public class TrianglePrismChamferAdjacencyParityLabTests
{
    private static string Stable(TrianglePrismChamferAdjacencyParityRow r)
        => $"{r.CaseName}|{r.LegacyProduced}|{r.CandidateProduced}|{r.FeatureRecognitionParity}|{r.FirstDivergence}|{r.LegacyAdjacency.FaceCount}|{r.CandidateAdjacency.FaceCount}|{r.LegacyChamfer.CornerAdmissibleCount}|{r.CandidateChamfer.CornerAdmissibleCount}|{string.Join(',', r.Diagnostics)}|{r.Recommendation}";
