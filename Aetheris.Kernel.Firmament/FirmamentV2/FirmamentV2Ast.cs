...
public sealed record FirmamentV2Document(string ModelName, string Units, IReadOnlyList<FirmamentV2SolidBinding> Solids, IReadOnlyList<FirmamentV2ModifyBlock>? ModifyBlocks = null)
{
    ...
    public FirmamentV2SideHoleIntent? SideHoleIntent => ModifyBlocks?.SelectMany(m => m.Regions.Select(r => new FirmamentV2SideHoleIntent(m.TargetSolid, r.Name, r.Attachment.Axis, r.Cut.Tool.Through.Axis, r.Cut.Tool.ToolType, r.Cut.Tool.Radius, Units))).SingleOrDefault();
}
...
public sealed record FirmamentV2CylinderTool(string ToolType, double Radius, FirmamentV2FaceSelector Through);
public sealed record FirmamentV2SideHoleIntent(string TargetSolid, string RegionName, string AttachFace, string ThroughFace, string Tool, double Radius, string Units);
