...
public sealed class FirmamentV2ConstructionPlaneHoleSourceTests
{
    ...
    {
        var source = File.ReadAllText(Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../fixtures/Regression/Hole/valid/construction-plane-blind-drillpoint-shaft-depth.firmament")));
    ...
    private static string Fixture => Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../fixtures/Regression/Hole/valid/construction-plane-through-hole.firmament"));
    private static string InvalidFixture(string name) => Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../../fixtures/Invalid/Hole", name));
}
