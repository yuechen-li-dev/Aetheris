...
- `ADVANCED_FACE.surface` accepts either a `#entity` reference or a limited inline surface constructor (`PLANE(...)`, `CYLINDRICAL_SURFACE(...)`, `CONICAL_SURFACE(...)`, and `SPHERICAL_SURFACE(...)` in M47d).
...
- Inline surface constructors are intentionally limited in v0: `ADVANCED_FACE.surface = PLANE(...)`, `CYLINDRICAL_SURFACE(...)`, `CONICAL_SURFACE(...)`, and `SPHERICAL_SURFACE(...)` are accepted in M47d; all other inline surface constructors fail deterministically via `Importer.StepSyntax.InlineEntity`.
