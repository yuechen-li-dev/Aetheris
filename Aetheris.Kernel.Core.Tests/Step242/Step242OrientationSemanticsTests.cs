...
public sealed class Step242OrientationSemanticsTests
{
    ...
    [Fact]
    public void ImportBody_EdgeCurveSameSenseFalse_WithOppositeLineDirection_ImportsSuccessfully()
    {
    ...
    [Fact]
    public void ImportBody_EdgeCurveSameSenseTrue_WithOppositeLineDirection_ReturnsGeometryDiagnostic()
    {
        var result = Step242Importer.ImportBody(BuildSingleEdgeStep(edgeCurveSameSense: true, orientedEdgeOrientation: true, lineDirectionX: -1d));

        Assert.False(result.IsSuccess);
        var first = Assert.Single(result.Diagnostics);
        Assert.Equal("Importer.Geometry.EdgeCurveParameters", first.Source);
        Assert.Contains("opposite to vertex ordering", first.Message, StringComparison.Ordinal);
    }
    ...
    private static string BuildSingleTriangleStep(bool edgeCurveSameSense, bool faceBoundOrientation, bool advancedFaceSameSense)
    {
        ...
        return BuildSingleEdgeStep(edgeCurveSameSense, orientedEdgeOrientation: true, lineDirectionX: 1d, boundOrientation, faceSense);
    }

    private static string BuildSingleEdgeStep(bool edgeCurveSameSense, bool orientedEdgeOrientation, double lineDirectionX, string boundOrientation = ".T.", string faceSense = ".T.")
    {
        var edgeSense = edgeCurveSameSense ? ".T." : ".F.";
        var orientedSense = orientedEdgeOrientation ? ".T." : ".F.";

        return $"ISO-10303-21;\nHEADER;\nENDSEC;\nDATA;\n#1=MANIFOLD_SOLID_BREP('solid',#2);\n#2=CLOSED_SHELL($,(#3));\n#3=ADVANCED_FACE((#4),#5,{faceSense});\n#4=FACE_OUTER_BOUND($,#6,{boundOrientation});\n#5=PLANE($,#20);\n#6=EDGE_LOOP($,(#7));\n#7=ORIENTED_EDGE($,$,$,#8,{orientedSense});\n#8=EDGE_CURVE($,#9,#10,#11,{edgeSense});\n#9=VERTEX_POINT($,#12);\n#10=VERTEX_POINT($,#13);\n#11=LINE($,#12,#14);\n#12=CARTESIAN_POINT($,(0,0,0));\n#13=CARTESIAN_POINT($,(1,0,0));\n#14=VECTOR($,#15,1.0);\n#15=DIRECTION($,({lineDirectionX},0,0));\n#20=AXIS2_PLACEMENT_3D($,#12,#21,#22);\n#21=DIRECTION($,(0,0,1));\n#22=DIRECTION($,(1,0,0));\nENDSEC;\nEND-ISO-10303-21;";
    }
