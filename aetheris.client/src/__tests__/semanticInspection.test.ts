...
const artifact: CadmataVisualizationArtifact = {
	...
	entities: [
		...
		},
		{
			...
			label: "MountDiameter",
			layer: "conceptPoints",
			geometry: { type: "circle", center: { x: 1, y: 2, z: 3 }, radius: 4 },
			metadata: {
				targetSemanticId: "hole",
				nominal: "8 mm",
				tolerancePlus: "0.05 mm",
				toleranceMinus: "0.02 mm",
			},
		},
	],
};
...
describe("semantic inspection index", () => {
	it("keeps compiler-published face owners and deterministic hierarchy", () => {
		expect(
			indexSemanticInspection(artifact)
				.faceOwners.get(17)
		...
		expect(indexSemanticInspection(artifact).pmiByTarget.get("hole")?.[0].label).toBe(
			"MountDiameter",
		);
	});
