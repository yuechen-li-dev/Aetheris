...
Artifacts under `docs/continuum/artifacts/attention-e0/` include benchmark JSON/CSV, full residual histories, operator configurations and rejection reasons, mode analysis, mathematical contracts, interaction weights, a timing-free deterministic result projection, and its SHA-256 hash. Runtime fields are deliberately excluded from the deterministic hash.
