...
public sealed record FirmamentV2ThreadDecl(string Name, string SupportSolid, string SupportRole,
    double MajorDiameterMm, double PitchMm, double LengthMm, double? StartOffsetMm,
    string Hand, FirmamentV2SourceSpan SourceSpan);
...
/// <summary>One qualified, deterministic, oriented boundary before its syntax is erased.</summary>
public sealed record FirmamentV2ClosedBoundary2Decl(
    string Name,
    string ShapeType,
    string? Variant,
    string Capability,
...
public sealed record FirmamentV2RadialPatternDecl(string Name, string Source, string Axis, int Count, double AngleRadians, string Distribution, IReadOnlyList<FirmamentV2RadialInstance> Instances, FirmamentV2SourceSpan SourceSpan);
public sealed record FirmamentV2RadialInstance(string Identity, int InstanceIndex, double RotationAngleRadians, double CenterX, double CenterY, IReadOnlyList<string> Provenance);
