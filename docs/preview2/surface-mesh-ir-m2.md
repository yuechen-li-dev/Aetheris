...
aetheris mesh fixtures/FirmamentV2/Hole/valid/hole-x4-shaft-through.valid.firmfixture --output plate.stl --debug-ir plate.ir.json --json
```
...
The stored [IR dump](evidence/surface-mesh-ir-m2/through-hole.ir.json), [STL](evidence/surface-mesh-ir-m2/through-hole.stl), and [FreeCAD smoke script](evidence/surface-mesh-ir-m2/freecad-stl-smoke.py) are reproducible evidence. FreeCAD 1.0.2 imported the STL as one solid mesh: 504 facets, 252 points, `solid=True`, 100 x 60 x 12 mm bounds, and volume 72,200.078125 mm³ (binary STL float rounding). No repair/healing was requested.
