	...
	const handleFiles = useCallback(
		async (files: File[]) => {
			...
			if (!metadataResult.success) {
				const message =
					metadataResult.error.issues[0]?.message ?? "Unable to read STEP file metadata.";
				setSelectionMetadata(null);
				setSelectionError(message);
				onValidationError(message);
				return;
		...
		<>
			...
			>
				{selectionError ? (
					...
					<div className="step-dropzone__content">
						<p className="step-dropzone__primary" title={selectionMetadata.fileName}>
							{selectionMetadata.fileName}
						</p>
