...
EDGE-A1 maps this gated stability check to the artifact-corpus and artifact-stability readiness gates in `docs/development/milestones/general/edge-a1-chamfer-fillet-support-compatibility-matrix.md`. Passing EDGE-X12 stability keeps the controlled AirChamfer corpus repeatable, but it remains an opt-in evidence gate and does not replace legacy chamfer/fillet production routes.
