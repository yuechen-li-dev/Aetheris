...
This milestone is the first implementation slice for `docs/development/scope-contracts/mvp/firmament-v2-ap242-mvp-readiness-contract.md`: a Firmament V2 fixture is considered ready only when real AP242 is emitted from a real `BrepBody` by `Step242Exporter` and independently verified by reimport, topology checks, and volume checks.

## Forward link

STEP-V2-X1 extends this Box-only bridge to Cylinder, Cone/frustum, Sphere, and Torus while preserving the V2-to-lowered compatibility seam; see `docs/development/implementation/step-v2-x1-analytic-primitives-ap242.md`.
