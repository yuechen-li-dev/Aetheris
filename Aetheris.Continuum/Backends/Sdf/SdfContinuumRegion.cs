...
public sealed class SdfContinuumRegion : IContinuumRegion, IImplicitFieldCapability, IGradientCapability, IBoundsClassificationCapability
{
    private readonly SdfTape tape;

    public SdfContinuumRegion(RegionId id, SdfNode root)
    {
        Id = id;
        Root = root ?? throw new ArgumentNullException(nameof(root));
        tape = SdfTapeLowerer.Lower(root);
        Capabilities = SdfCapabilityAnalyzer.Analyze(root);
        Bounds = new BoundingBox3D(root.Bounds.Min, root.Bounds.Max);
    }

    public RegionId Id { get; }
    public SdfNode Root { get; }
    public BoundingBox3D Bounds { get; }
    public SdfFieldCapabilities Capabilities { get; }

    public double FieldValue(Point3D point) => tape.Evaluate(point);
    ...
    {
        var interval = tape.EvaluateInterval(new SdfBounds(bounds.Min, bounds.Max));
        var center = new Point3D(
            (bounds.Min.X + bounds.Max.X) * 0.5d,
            (bounds.Min.Y + bounds.Max.Y) * 0.5d,
            (bounds.Min.Z + bounds.Max.Z) * 0.5d);
        if (interval.MaxValue <= tolerance && FieldValue(center) <= 0d)
        {
            return ContinuumBoundsClassification.Inside;
        }

        if (interval.MinValue >= -tolerance && FieldValue(center) > 0d)
        {
    ...
    public bool TryGradient(Point3D point, out Vector3D gradient)
    {
        ...
        gradient = new Vector3D(
            (FieldValue(new Point3D(point.X + h, point.Y, point.Z)) - FieldValue(new Point3D(point.X - h, point.Y, point.Z))) / (2d * h),
            (FieldValue(new Point3D(point.X, point.Y + h, point.Z)) - FieldValue(new Point3D(point.X, point.Y - h, point.Z))) / (2d * h),
            (FieldValue(new Point3D(point.X, point.Y, point.Z + h)) - FieldValue(new Point3D(point.X, point.Y, point.Z - h))) / (2d * h));
