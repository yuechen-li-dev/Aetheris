...
The current AirChamfer golden path is strong but narrow: controlled convex planar single-edge chamfers now have Judgment-backed policy, topology planning, geometry artifacts, closed witnesses, controlled topology graft evidence, a production-adjacent prototype, feature-recognition parity probes, a non-authoritative shadow route, CLI STEP artifacts/corpus coverage, and gated corpus stability evidence.
...
| CH-03 | convex/replacement | plane-plane, orthogonal controlled body | single edge | constant chamfer distance | no corner | plane | `production-legacy-supported` for legacy bounded box/simple contexts | `production-adjacent-air-prototype`; `shadow-supported`; `artifact-corpus-supported`; `closed-witness-supported` | EDGE-X3 through EDGE-X12, EDGE-V1/V2/V3, CLI tests, FrictionLab AirChamfer tests | production authority, arbitrary body mutation, route fallback policy | Best current AirChamfer row, still non-authoritative | EDGE-V4 controlled opt-in route only after diagnostics remain stable |
...
Recommended post-EDGE-X12 roadmap:

1. **EDGE-X13**: non-authoritative Firmament shadow diagnostics probe, if desired, to capture production-adjacent diagnostics without changing route authority.
