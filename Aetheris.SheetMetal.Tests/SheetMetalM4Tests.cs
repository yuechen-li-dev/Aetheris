...
public sealed class SheetMetalM4Tests
{
    ...
    {
        var path=Path.Combine(RepoRoot,"fixtures/Regression/CanonicalGeometry/power-supply-enclosure-m4.firmament");var source=File.ReadAllText(path);var result=SheetMetalFirmament.CompileFile(path);
    ...
    {
        var bad=SheetMetalFirmament.CompileFile(Path.Combine(RepoRoot,"fixtures/Invalid/SheetMetal/tray-dfm-violation.firmament"));var fixedPart=SheetMetalFirmament.CompileFile(Path.Combine(RepoRoot,"fixtures/Canonical/SheetMetal/tray-dfm-policy.firmament"));Assert.True(bad.IsSuccess);Assert.True(fixedPart.IsSuccess);
