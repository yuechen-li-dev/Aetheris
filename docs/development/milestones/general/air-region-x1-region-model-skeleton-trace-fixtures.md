...
Parser-backed `fixtures/Primitive/valid/box.valid.firmfixture` now reports a single `RootRegion` with a `WorldRoot` frame, `PureConstruction` effect, `YieldBody`, `YieldsBody`, and `NotRequired` integration. The fixture still reaches `emitted-brep` through the existing AIR-X11 box path.
