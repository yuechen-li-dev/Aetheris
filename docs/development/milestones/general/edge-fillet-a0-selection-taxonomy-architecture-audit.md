...
AIR-A0 places future fillet support behind AIR route selection: constant-radius Class A evidence should lower through the highest construction-aware route available, then a future `AirFillet`/`AirEdgeSweep` local route only when no-history support is explicitly admitted, with legacy bounded BRep fillet behavior preserved as the current authoritative fallback. See `docs/development/milestones/general/air-a0-aetheris-v2-compiler-ir-constitution.md`.
