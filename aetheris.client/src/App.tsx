                        ...
                        <>
                            <section className="tool-section cadmata-inspector">
                                ...
                                <div className="stack-row">
                                    {['direct-profile', 'split-compose-chamfer', 'semantic-shaft-hole', 'construction-plane-blind-drillpoint', 'ctc-01-x3', 'ctc-01-x4'].map((fixtureId) => <Button key={fixtureId} type="button" variant="outline" onClick={() => void handleLoadCadmataFixture(fixtureId)} disabled={!documentId || status === 'loading'}>{fixtureId}</Button>)}
                                </div>
                                {cadmataArtifact ? <>
