...
The current geometry was frozen under `artifacts/release/p1-harden-m2-baseline/`. No Fillet construction, topology, policy, or STEP exporter code changed. A narrow Green-theorem lane now integrates single-loop horizontal planar faces bounded by exact lines, circles, and ellipses. Permanent reimport tests cover both seven-station policies.
