...
EDGE-A3 adds a selection-pattern taxonomy over the matrix: baseline edge-finish support should be interpreted as selection-pattern driven, with Tier 1 single-edge, Tier 2 face-boundary loop, and Tier 3 whole-body canonical edge-set cases prioritized under uniform symmetric chamfer or constant-radius fillet rules. Arbitrary edge graphs, mixed per-edge rules, unequal-distance chamfers, and variable-radius fillets remain deferred/advanced unless a future bounded policy admits them. See `docs/development/milestones/general/edge-a3-edge-finish-selection-taxonomy.md`.

EDGE-FILLET-A0 applies that taxonomy specifically to fillets. It keeps existing legacy bounded fillet behavior authoritative where production support exists, selects `EDGE-FILLET-X1 — Single plane-plane constant-radius fillet cylindrical witness` as the preferred first new proof, scopes baseline fillets to constant-radius uniform rules, and defers loop/body fillets until single-edge/corner evidence exists. See `docs/development/milestones/general/edge-fillet-a0-selection-taxonomy-architecture-audit.md`.
...
## EDGE-FILLET-A0 status

EDGE-FILLET-A0 adds a documentation-only fillet architecture audit using the EDGE-A3 selection taxonomy. It recommends constant-radius fillets as the baseline rule, classifies Class A single-edge, Class B face-boundary loop, Class C whole-body canonical, and Class D arbitrary graph fillet selections, and selects `EDGE-FILLET-X1 — Single plane-plane constant-radius fillet cylindrical witness` as the preferred first proof. Existing `BrepBoundedFillet` production behavior remains authoritative for admitted bounded manufacturing scopes; no implementation, production route replacement, public API, STEP, Boolean, BRep topology, AIR emitter, CIR/analyzer, variable-radius, arbitrary-graph, or NURBS/freeform behavior changed.
