...
|---|---|---|---|
| First exact part | `fixtures/Canonical/valid/bare-box.firmament` | `aetheris build` | passed; authoritative volume assertion passed |
| Table/Template/Profile | `fixtures/Canonical/valid/table-template-concept-path-compose.firmament` | `aetheris build` | passed |
| HexBolt standards family | `fixtures/LegacyV1/Examples/hexbolt_template_m2.firmament` | `aetheris build` + `aetheris mesh --format obj --debug-ir` | passed; watertight SurfaceMeshIR output |
| Profile bracket | `fixtures/Canonical/valid/profile-compose-l-bracket-counterbore-pmi.firmament` | `aetheris build` | passed; datum and diameter PMI evidence found |
| Imported STEP | `fixtures/Canonical/valid/inline-step-recognize-replace.firmament` | `aetheris build` | passed; one region recognized/replaced and hybrid STEP verified |
