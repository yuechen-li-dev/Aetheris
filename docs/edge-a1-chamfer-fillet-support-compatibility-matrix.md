...
| CH-03 | convex/replacement | plane-plane, orthogonal controlled body | single edge | constant chamfer distance | no corner | plane | `production-legacy-supported` for legacy bounded box/simple contexts | `production-adjacent-air-prototype`; `shadow-supported`; `artifact-corpus-supported`; `closed-witness-supported` | EDGE-X3 through EDGE-X13, EDGE-V1/V2/V3, CLI tests, FrictionLab AirChamfer tests, Firmament shadow-diagnostics tests | production authority, arbitrary body mutation, route fallback policy | Best current AirChamfer row, still non-authoritative; EDGE-X13 adds Firmament-facing test-only shadow diagnostics | EDGE-V4 controlled opt-in route only after diagnostics remain stable |
