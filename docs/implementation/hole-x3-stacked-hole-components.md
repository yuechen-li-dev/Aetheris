...
This implements the principle from `docs/llm-cad-strategy/07-holes-are-semantic-features.md`: counterbores and countersinks are variants of a semantic hole feature. The entry preparation and shaft remain an owned axial feature stack because later standards, PMI, manufacturing intent, and edits need a single parent hole identity.
